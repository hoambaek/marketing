...
export default function UAPSPage() {
  ...
  // before = 지상 대조군 축별 중앙값, after = before + 시음자별 쌍대차(actual−terrestrial) 중앙값
  // → 시음자별 척도 편차(대조군 flat 50 등)를 각자 baseline으로 상쇄. 평균보다 견고.
      ...
      <AnimatePresence>
        ...
        <SectionWrapper title="풍미 프로파일" icon={BarChart3} iconColor="#B76E79" delay={0.3}>
          <FlavorRadar
            ...
            beforeLabel={measuredProfiles ? '숙성 전(지상)' : undefined}
            afterLabel={measuredProfiles ? '숙성 후(해저 실측)' : undefined}
          />
        </SectionWrapper>
