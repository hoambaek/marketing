...
function buildExpertProfilePrompt(product: AgingProduct, monthlyOceanProfiles?: MonthlyOceanProfile[] | null): string {
  ...
  const expertRoles: Record<string, string> = {
    ...
    sake: '사케 전문가 (唎酒師)',
    whisky: '위스키 전문가',
    yakju_cheongju: '전통주 전문가 (약주·청주, 발효주 소믈리에)',
    spirits: '증류주 전문가 (증류식 소주·전통 리큐르)',
    puer: '보이차/생차 전문가 (茶藝師)',
    soy_sauce: '발효 식품 전문가',
  ...
  const sourcesByCategory: Record<string, string> = {
    ...
    yakju_cheongju: '더술닷컴, 전통주갤러리, 우리술닷컴, 대한민국 우리술 품평회',
    spirits: '더술닷컴, 증류식소주 리뷰, 대한민국주류대상',
...
const EXPERT_ROLES: Record<string, string> = {
  ...
  green_coffee_bean: '생두 숙성 과학 전문가 (SCA Q-Grader, green grading)',
  sake: '사케 숙성 과학 전문가 (唎酒師)',
  whisky: '위스키 숙성 과학 전문가 (Master of Whisky)',
  yakju_cheongju: '전통주 발효·숙성 과학 전문가 (약주·청주)',
  spirits: '증류주 숙성 과학 전문가 (증류식 소주)',
...
- ⏱️ timeScale (숙성 시간축 압축, 0.3~5.0): 이 제품군이 **얼마나 빠르게 성숙 피크에 도달하는지**의 배수입니다.
  기본 물리 곡선은 와인/샴페인의 수년 단위 숙성을 가정하므로, 수개월에 익는 제품은 timeScale로 시간축을 압축해야 곡선 피크가 실제 최적 인양 시점과 일치합니다.
  기준: 샴페인·와인 = 1.0 / 사케·위스키 ≈ 1.0~1.5 / 전통주(약주·청주)·간장·식초 ≈ 3~4(수개월 성숙) / 생두 ≈ 1.5~2.5 / 보이차 < 1.0(수년 후발효).
  ⚠️ harvestWindow(최적 인양 개월)와 timeScale이 **서로 모순되지 않게** 정하세요. 예: 최적 인양이 4~6개월이면 timeScale은 3~4 수준이어야 합니다.
...
function clampPredictionValues(response: AIPredictionResponse): AIPredictionResponse {
  ...
  if (response.agingFactors) {
    ...
    agingFactors = {
      ...
      timeScale: Math.min(AGING_FACTORS_RANGE.timeScale.max, Math.max(AGING_FACTORS_RANGE.timeScale.min, af.timeScale ?? DEFAULT_AGING_FACTORS.timeScale ?? 1.0)),
    };
  }
